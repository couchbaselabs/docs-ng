...
This section only gives an introduction to the APIs available. The actual API
reference will feature more options for each of the APIs described here and will
...
While this has been discussed extensively in previous sections, you can connect
to a bucket using the simple `Couchbase.connect()` class method.
...
Sometimes, it might be beneficial to let the client know beforehand about
multiple nodes; for example if you have several nodes in the cluster and some
may not be up; if only one node is passed, the client’s constructor will raise
an exception. You can pass multiple nodes as a list so that the constructor will
try each node until it gets a successful connection (or the timeout is reached)
...
The client uses timeouts so that your application will not wait too long if the
...
If your bucket is password protected, you can pass the SASL password using the
`password` keyword parameter in the constructor
...
This will be discussed later on in more detail, but the `Connection` object is
fully thread-safe out of the box by default. You may tune some parameters which
sacrifice thread-safety for performance.
 ...
 * `rc`
   ...
   layer. This is 0 on success and nonzero on failure. Typically this will be
   useful on operations in which `quiet` was set to `True`. Normally you’d use it
   like this
...
fail, they will raise an exception (and are not affected by the `quiet`
property).
...
indicates the lifetime of the value; after the `ttl` is reached, the value is
 ...
 * `client.add(key, value, **kwargs)`
   ...
   Will set the key to the specified value, but only if the key does not already
   exists (an exception will be raised otherwise).

 * `client.replace(key, value, **kwargs)`

   Will replace an existing key with a new value. This will raise an exception if
...
mutation and retrieval of values. You may use these in place of the `set` family
...
values, as well as an `amount` value which indicates by what amount to modify
their values. Additionally, an `initial` keyword argument is available to
provide the default value for the counter if it does not yet exists. If an
...
to it. They treat existing values as strings and such only work if the existing
data stored is a string (i.e. `FMT_UTF8` or `FMT_BYTES` ).
...
This consists of a single method which is used to update the expiration time of
a given key. It is passed two arguments, a key and an expiration time.
...
overhead of having to re-set their value or fetch them.
...
This returns a `ValueResult` object (subclass of `Result` ) which may be used to
...
called *Opportunistic Locking*, while the second is called *Pessimistic
Locking*.
...
application, however it can be used to ensure consistency. You may pass the
...
In the previous example (i.e. `add_friend` ), we can now modify it so that it
...
Pessimistic locking is useful for highly contented resources; that is, if the
key being accessed has a high likelihood of being contented. While this method
...
Once the key is locked, further attempts to access the key (without passing the
proper CAS) will fail with a `TemporaryFailureError` exception until the key is
either unlocked, or the lock timeout is reached.
...
**Unhandled:** `[:unknown-tag :important]` We can rewrite our `add_friend`
...
impacts on network I/O, as the value must be sent multiple times before it is
actually stored.
...
the Python SDK. If you are new to views, it is recommended you read the server
documentation <link?> which covers the topic itself more extensively.
...
the Python SDK (see <link> design document management functions).
...
**Unhandled:** `[:unknown-tag :screen]` Which returns an iterable object which
 ...
 * `vr.doc`
   ...
   passed the `include_docs` directive (see later).
...
are documented in the `Query` class in the API documentation.

**Unhandled:** `[:unknown-tag :screen]` The `include_docs` directive may be used
to fetch the documents along with each `ViewRow` object. Note that while it is
possible to simply call `c.get(vr.id)`, the client handles the `include_docs`
directive by actually performing a batched ( `get_multi` ) operation.

You can also pass options for the server itself to handle. These options may be
...
Note that this is the most efficient way to pass options as they do not need to
be re-encoded for each invocation.
...
means that if you try to use the view right after you created it, you may get an
error as the operation has not yet completed. Using the `syncwait` parameter
will poll for this many seconds - and either return successfully or raise an
exception.
 ...
 * `c.design_create(name, design, use_devmode=True, syncwait=0)`
   ...
   `"beer"` ). `design` is either a Python dictionary representing the structure of
   the design or a valid string (i.e. encoded JSON) to be passed to the server.

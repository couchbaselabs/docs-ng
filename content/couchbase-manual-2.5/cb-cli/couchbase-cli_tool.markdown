...
`group-manage`         | Manages server groups      
...
You can also perform many of these same settings using the REST API, see the [Couchbase REST API](cb-restapi#couchbase-admin-restapi).
...
To add a node to a cluster, but do not rebalance:
...
To add a node to a cluster and rebalance:
...
To remove a node from a cluster and rebalance:
...
To remove and add nodes from/to a cluster and rebalance:
...
To stop the current rebalancing:

```
couchbase-cli rebalance-stop -c 192.168.0.1:8091
```

To set the username, password, port and ram quota:
...
To change the cluster username, password, port and ram quota:

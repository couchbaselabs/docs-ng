...
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
Meta data per document (metadata\_per\_document)                                                                                                                                                                            | This is the space that Couchbase needs to keep metadata per document. Prior to 2.1, it is 64 bytes. As of Couchbase 2.1 metadata uses 56 bytes of memory. All the metadata for documents needs to live in memory while a node is running and serving data.
...
There are several ways you can provide hostnames for Couchbase 2.1+. You should
set the hostname before you add the node to a cluster. You can provide a
hostname when you install a Couchbase Server 2.1 node, as you add it to an
existing cluster for online upgrade, or via a REST-API call before the node is
part of a cluster. If you restart, any hostname you establish with one of these
methods will be used. For instructions, see [Using Hostnames with Couchbase
...
memory are moved to the swap space. Swappiness indicates how frequently a system
should use swap space based on RAM usage and is a value between 0 to 100. We
have the following recommendations for swap space:
 ...
 * If you use Couchbase Server 2.0+ with views, we recommend setting swappiness to
   0 or your system swap usage will be far too high.

 * Certain cloud systems by default do not have a swap partition configured. If you
   are using views, we recommend setting swapiness to 0 and if you are not using
   views, you can set this to 10.

To view the currently set swapiness on your system:
...
end of the file. After save this and reboot your system, this setting will be
used.
